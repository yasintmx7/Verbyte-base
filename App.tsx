...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const onPlayerJoined = (id: string, joiner: string) => {
      if (id === roomId && joiner.toLowerCase() !== account?.toLowerCase()) {
        addLog("Rival node detected. Synchronizing...", "success");
        setGameState(prev => ({
          ...prev,
          players: [...prev.players.filter(p => !p.isBot), {
            id: joiner,
    ...
    const onMoveMade = (id: string, player: string, letter: string, isHit: boolean) => {
      if (id !== roomId) return;

      // If it's the opponent's move, update UI
      if (player.toLowerCase() !== account?.toLowerCase()) {
        addLog(`Opponent decrypted: ${letter}`, "warn");

        setGameState(prev => {
          const cleanLetter = letter.toUpperCase();
          if (prev.guessedLetters.includes(cleanLetter)) return prev;

          // Update Board
          return {
            ...prev,
            guessedLetters: [...prev.guessedLetters, cleanLetter]
          };
        });
      }
    };
  ...
  const createPrivateRoom = async () => {
    ...
    const wordData = await fetchRandomWord(); // Word is generated by Host

    addLog("Initializing On-Chain Session...", "warn");

    try {
      const tx = await contract.createGame(newId, wordData.word);
      addLog("Confirming room creation...", "info");
      await tx.wait();
      ...
      // Start Local State
      setGameState({
        status: GameStatus.LOBBY, // Stay in Lobby for now? Or wait for Join?
        // Requirement: "Players can play". Host enters immediately.
        // Let's enter Matchmaking/Waiting state.
        players: [{
          id: account,
          name: 'Host',
          avatar: `https://api.dicebear.com/7.x/pixel-art/svg?seed=${account}`,
          hp: INITIAL_HP,
          score: 0,
          isReady: true
        }],
      ...
      startMatchmaking(`https://api.dicebear.com/7.x/pixel-art/svg?seed=${account}`); // Re-use existing flow but now backed by ID

      addLog(`Room ${newId} Created On-Chain!`, 'success');

    } catch (e: any) {
      console.error(e);
      addLog(`Creation failed: ${e.reason || e.message}`, 'error');
    }
  };
  ...
  const startMatchmaking = async (avatar: string) => {
    ...
    if (roomId && contract) {
      addLog(`Initiating connection to Node ${roomId}...`, "warn");
      try {
        // Attempt to join. If we are creator, this will fail (revert), which is fine.
        // We can check if we are already Player 1?
        // Or just Try/Catch.
        const tx = await contract.joinGame(roomId);
        addLog("Handshake protocol active...", "info");
        await tx.wait();
        addLog("Uplink established.", "success");
      } catch (e: any) {
        // Ethers error parsing is messy, but if it's "Cannot play self", we are host.
        // Or if "Game not waiting", maybe it's full.
        console.log("Join skipped/failed (likely Host):", e);
        // We assume valid flow for now.
      }
    }
    ...
    if (!roomId) {
      ...
      // Multiplayer: Just wait for Event "PlayerJoined" (handled in useEffect) to trigger UI update.
      // We do typically want to enter "Playing" state once needed.
      // But for now, Matchmaking screen is fine until rival appears.
      startGame([player]); // Start immediately with just us? 
      // Wait, if we start game, we see board.
      // Yes, start game, opponent will appear via event listener.
    }
  };
  ...
  const handleGuess = useCallback(async (letter: string) => {
    ...
    if (roomId && contract) {
      try {
        ...
        contract.submitMove(roomId, cleanLetter).then((tx: any) => {
          addLog(`Transmission sent: ${cleanLetter}`, "info");
          tx.wait().then(() => addLog(`Block confirmed: ${cleanLetter}`, "success"));
        });
      } catch (e) {
        console.error(e);
      }
  ...
  }, [account, addLog, gameState.status, gameState.isStunned, gameState.guessedLetters, gameState.word, gameState.players, roomId, contract]);
